...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \begin{tikzpicture}
            \begin{axis}[xmode=log, ymode=log, grid=major,
                        xlabel = {\footnotesize \Forest},
                        ylabel = {\footnotesize \Regel},
                        xmax = 5000 ,ymax = 5000,
    ...
    %         \begin{axis}[xmode=log, ymode=log, grid=major,
    %                     xlabel = {\footnotesize \Forest},
    %                     ylabel = {\footnotesize \Regel w/o sketches},
    %                     xmax = 5000 ,ymax = 5000,
    %                     width=.99\linewidth, height=.99\linewidth,
    %                     clip marker paths=true]
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \begin{tikzpicture}
            \begin{axis}[xmode=log, ymode=log, grid=major,
                        ...
                        width=.99\linewidth, height=.99\linewidth,
                        clip marker paths=true]
            \addplot[thick, only marks, mark=x, fill opacity=0.6, draw opacity=0.9, color=red] table[x=regex_synthesis_time, y=regel_time, col sep=comma] {data/regel_comparison_dyn.csv};
            \addplot[thick, only marks, mark=x, fill opacity=0.6, draw opacity=0.9, color=blue] table[x=regex_synthesis_time, y=regel_time, col sep=comma] {data/regel_comparison_sta.csv};
            \addplot[domain=0.01:3600,smooth,thin]{x};
            \addplot [dotted, thick, timeout-color, opacity=0.8] coordinates {(0.01,3600) (3600,3600)}
                node at (3,2500) {\tiny{3600 second timeout}};
            \addplot [dotted, thick, timeout-color, opacity=0.8] coordinates {(3600,0.01) (3600,3600)}
